...
DATA_DIR=/lustre/scratch117/sciops/team233/Erica/ATAC_seq_PTK2B_2022_last
SAMPLE=${2}
SAMPLENUMBER=${SAMPLE}
FRACTION=${3}
...
case $1 in
  ...
  readsinbam)
    	...
    	echo -e "${SAMPLENUMBER}\t${READS}" >>read_counts_atac_micro_ptk2b.tsv
    	;;
  index)
...
#	;;
  ...
  mergebeds)
	...
	shift
	BEDSTOMERGE=("$@")
	OUTPUTFILE=${DATA_DIR}/counts/ATAC_narrowpeaks_allMerged.bed
	...
	cat ${BEDSTOMERGE[@]} | sort -k1,1 -k2,2n | mergeBed -i stdin >${OUTPUTFILE}
    	;;
  sort_mergedbed)
	bedtools sort -faidx /lustre/scratch117/sciops/team233/Erica/reference/chromosome.sizes -i ${DATA_DIR}/counts/ATAC_narrowpeaks_allMerged.bed > ${DATA_DIR}/counts/ATAC_narrowpeaks_allMerged_sorted.bed &
	;;
  generate_counts)
	echo "Getting counts under peaks for sample ${SAMPLE}"
	coverageBed -a ${DATA_DIR}/counts/ATAC_narrowpeaks_allMerged_sorted.bed -b ${DATA_DIR}/bams/${SAMPLE}_sorted_nodupl.bam -g /lustre/scratch117/sciops/team233/Erica/reference/chromosome.sizes -sorted -counts > ${DATA_DIR}/counts/${SAMPLE}_ATAC_micro_PTK2B.counts &
	;;
   *)

...
setwd("Documents/ptk2b_kolf_B11_C8_D3/RNAseq/")
#import counts table from FeatureCounts output
read.table("featurecounts/counts_ptk2b_110629.txt" , header = T, stringsAsFactors = F, sep = "\t") -> fc_table
head(fc_table)

# to remove .bam from the end but not used cause I need to add WT/HOM 
#a <- gsub("_macro_PTK2B.bam","",colnames(fc_table))
#colnames(fc_table) <- a
...
#make legend df
legend <- data.frame(row.names = colnames(counts_macro), genotype_PTK2B=as.factor(c(rep("WT",6), rep("HOM",9))), batch=as.factor(c(rep("2", 6), rep("1", 3), rep("2", 6))))
legend
...
write.csv(tpm_expressed, 
          file="/Users/eb19/Documents/ptk2b_kolf_B11_C8_D3/RNAseq/all_samples_noF4/PTK2B_macrophages_TPMs_all_noF4.csv")



#do DE for genes with at least 1 TPM (filter raw counts by tpm_expressed)
...
write.csv(counts_normalised, 
          file="/Users/eb19/Documents/ptk2b_kolf_B11_C8_D3/RNAseq/all_samples_noF4/PTK2B_macrophages_normalisedCounts_DESeq2_noF4.csv")
...
#batch correction on vsd tranformed counts for PCA plot
...
ggplot(data= df, mapping = aes(x=PC1 , y=PC2))+geom_point(size=4, mapping = aes(color=genotype_PTK2B, shape=hiPSC_clone))+ scale_color_manual(values=c("orange", "blue"))+scale_shape_manual(values=c(17, 18, 16,19, 15,20))+theme_classic()+theme(aspect.ratio = 1,legend.title = element_text(color = "black", size = 8),legend.text = element_text(color = "black", size = 6), legend.key.size = unit(0.3, "cm"),legend.margin=margin(0,0,0,0),
                                                                                                                                                                                                                                                   legend.box.margin=margin(0,0,0,0), legend.box.spacing = margin(10,10,10,10))+ xlab("PC1 65% variance") + ylab("PC2 29% variance")
#ggsave("/Users/eb19/Documents/ptk2b_kolf_B11_C8_D3/RNAseq/all_samples_noF4/PCA_vsdBatchCorr_macro_noNames.jpeg", device= "jpeg")

ggsave('../RNAseq/all_samples_noF4/PCA_vsdBatchCorr_macro_noNames.jpeg', device = "jpeg")


#batch correction of TPMs to make boxplots of individual genes
#calculated TPMs above from raw counts, excluded genes with TPM<1, now log10 transform TPMs and apply limma batch correction:
head(tpm_expressed)
#bind on first 2 columns of ENSEMBL and SYMBOLS
#log10 transform all TPMs
tpm_expressed_log10 <- cbind( tpm_expressed[,1:2], apply(tpm_expressed[,-c(1,2)], MARGIN=2, FUN= function(x) log10(x)))
#batch correction with limma
tpm_batch_corrected <- cbind(tpm_expressed[,1:2], limma::removeBatchEffect(tpm_expressed_log10[,-c(1,2)], batch=legend$batch, design = model.matrix(~legend$genotype_PTK2B)))
#re-transform 10^x 
tpm_batch_corrected <- cbind(tpm_batch_corrected[,1:2], apply(tpm_batch_corrected[,-c(1,2)], MARGIN=2, FUN= function(x) '^'(10,x)))
#some values are -Inf (NA) because they were 0 so log10 not possible and batch corr of NA not possible (but rowmean was > 1 so kept in)
#save batch corrected TPMs
fwrite(tpm_batch_corrected, file = "/Users/eb19/Documents/ptk2b_kolf_B11_C8_D3/RNAseq/all_samples_noF4/PTK2B_macrophages_TPMs_BATCHcorrected_all.csv")
...
write.csv(resultTable, 
          file="/Users/eb19/Documents/ptk2b_kolf_B11_C8_D3/RNAseq/all_samples_noF4/PTK2B_macrophages_resultsDESEq2Annotated_all_noF4.csv")
...
ggsave('all_samples_noF4/vulcanoPlot_allBatches_noF4.jpeg',device= "jpeg")
...
filter_DE_genes <- function (x, n, p) {
  ...
  fileName=paste0("/Users/eb19/Documents/ptk2b_kolf_B11_C8_D3/RNAseq/all_samples_noF4/PTK2B_macro_genes_up_down_p005_FC",n,".csv")
  write.csv(df, file=fileName)
  count <- df %>% count(diffExpr)
  fileName=paste0("/Users/eb19/Documents/ptk2b_kolf_B11_C8_D3/RNAseq/all_samples_noF4/PTK2B_res_macro_summary_p005_FC",n,".csv")
...
fileName=paste0("/Users/eb19/Documents/ptk2b_kolf_B11_C8_D3/RNAseq/all_samples_noF4/PTK2B_macrophages_",df_name,".csv")
